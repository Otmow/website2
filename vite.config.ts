...
export default defineConfig(({ command }) => ({
  ...
  //
  // Build only: in `vite dev` Node has node_modules, and forcing every dep
  // through the ESM module-runner makes CJS deps that reference bare `module`
  // throw "module is not defined" during SSR. Externalizing them in dev lets
  // Node load them natively.
  ssr: {
